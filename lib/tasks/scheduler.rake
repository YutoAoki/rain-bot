desc "This task is called by the Heroku scheduker add-on"
task :update_feed => :environment do
  ...
  client ||= Line::Bot::Client.new { |config|
     ...
     cnfig.channel_token = ENV["LINE_CHANNEL_TOKEN"]
   }
  ...
  if per06to12.to_i >= min_per || per12to18.to_i >= min_per || per18to24 >= min_per
    ...
    mid_per = 50
    if per06to12 >= mid_per || per12to18 >= mid_per || per18to24 >= mid_per
